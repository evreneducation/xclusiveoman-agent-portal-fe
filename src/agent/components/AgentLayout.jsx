...
function SidebarContent({ onNavigate, collapsed = false }) {
  ...
  return (
    <>
      <div className={`flex items-center gap-2.5 border-b border-white/10 px-6 py-6 ${collapsed ? 'justify-center px-3' : ''}`}>
        <img
          src="/Xclusive_Oman_Logo_2.png"
          alt="Xclusive Oman"
          className={`w-auto flex-none object-contain ${collapsed ? 'h-7' : 'h-12'}`}
        />
        {!collapsed && (
          <span className="flex-none rounded-full border border-agent-accent/40 bg-agent-accent-soft px-2 py-0.5 text-[9px] font-semibold uppercase text-agent-accent-dark">
            Agent Portal
          </span>
        )}
      </div>
